--[[
    ...
    Author: Haalfi.
    License: MIT
    Github: https://github.com/haalfiperth/Elisium
--]]
...
Elisium.DragBlacklist = {};
Elisium.IaDrag = false;
Elisium.LastDrag = tick();
Elisium.Flags = {};
...
Elisium.Lucide = {
	...
	["lucide-visuals"] = "rbxassetid://8547254518",
...
function Elisium:_GetIcon(name : string) : string
	return Elisium.Lucide['lucide-'..tostring(name)] or Elisium.Lucide[name] or Elisium.Lucide[tostring(name)] or name;
end;

function Elisium:_RandomString() : string
...
function Elisium:_IsMouseOverFrame(Frame : Frame) : boolean
	if not Frame then
		return;
	end;
...
function Elisium:_Rounding(num: number, numDecimalPlaces: number) : number
	local mult: number = 10 ^ (numDecimalPlaces or 0);
	return math.floor(num * mult + 0.5) / mult;
end;

function Elisium:_Animation(Self: Instance , Info: TweenInfo , Property :{[K] : V})
...
function Elisium:_Input(Frame : Frame , Callback : () -> ()) : TextButton
...
function Elisium:GetCalculatePosition(planePos: number, planeNormal: number, rayOrigin: number, rayDirection: number) : number
	local n = planeNormal;
...
function Elisium:_Blur(element : Frame , WindowRemote) : RBXScriptSignal
	local Part = Instance.new('Part',Elisium.ArcylicParent);
	...
	Part.CanQuery = false;
	Part.CollisionGroup = Elisium:_RandomString();
	...
	DepthOfField.Name = Elisium:_RandomString();

	Part.Name = Elisium:_RandomString();
	...
	local UpdateFunction = function()
		...
		if IsWindowActive then
			...
			Elisium:_Animation(DepthOfField,TweenInfo.new(0.1),{
				NearIntensity = 1
			})

			Elisium:_Animation(Part,TweenInfo.new(0.1),{
				Transparency = 0.97,
		...
		if IsWindowActive then
			...
			local pos0 = Elisium:GetCalculatePosition(planeOrigin, planeNormal, ray0.Origin, ray0.Direction);
			local pos1 = Elisium:GetCalculatePosition(planeOrigin, planeNormal, ray1.Origin, ray1.Direction);
...
function Elisium:_AddDragBlacklist(Frame: Frame)
	local IsAdded = false;
	local BASE_TIME = 0.01;

	local SET_BLACKLIST = function(value)
		local index = table.find(Elisium.DragBlacklist , Frame);

		if value and not Elisium.IS_DRAG_MOVE then
			if not index then
				table.insert(Elisium.DragBlacklist,Frame);
			end;
		else
			if index then
				table.remove(Elisium.DragBlacklist,index);
			end;
		end;
	end;

	Frame.InputBegan:Connect(function(input)
		if Elisium:_IsMouseOverFrame(Frame) then
			SET_BLACKLIST(true)
		end;
	end);
	...
	UserInputService.InputChanged:Connect(function()
		if not Elisium:_IsMouseOverFrame(Frame) then
			SET_BLACKLIST(false);
		end
	end);
end;

function Elisium:_GetWindowFromElement(Element)
	for i,v : ScreenGui in next , Elisium.Windows do
...
function Elisium.__SIGNAL(default)
...
function Elisium:_Hover(Frame: Frame , OnHover: () -> any?, Release: () -> any?)
	Frame.MouseEnter:Connect(OnHover);

	Frame.MouseLeave:Connect(Release);
end;

function Elisium.__CONFIG(config , default)
	config = config or {};
...
function Elisium:Drag(InputFrame: Frame, MoveFrame: Frame, Speed : number)
	...
	local function updateInput(input)
		...
		Elisium:_Animation(MoveFrame,Tween,{
			Position = position
		});
	end;

	InputFrame.InputBegan:Connect(function(input)
		if (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) and #Elisium.DragBlacklist <= 0 then 
			dragToggle = true
			...
			input.Changed:Connect(function()
				if input.UserInputState == Enum.UserInputState.End then
					dragToggle = false;
					Elisium.IS_DRAG_MOVE = false;
				end
			end)
		end

		if not Elisium.IsDrage and dragToggle then
			Elisium.LastDrag = tick();
		end;

		Elisium.IaDrag = dragToggle;
	end)

	UserInputService.InputChanged:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch and #Elisium.DragBlacklist <= 0 then
			if dragToggle then
				Elisium.IS_DRAG_MOVE = true;
				updateInput(input)
			else
				Elisium.IS_DRAG_MOVE = false;
			end
		else
			if #Elisium.DragBlacklist > 0 then
				dragToggle = false
				Elisium.IS_DRAG_MOVE = false;
			end
		end
...
function Elisium:_IsMobile()
	return UserInputService.TouchEnabled;
end;

function Elisium:_AddLinkValue(Name , Default , GlobalBlock , LinkValues , rep , Signal)
	if Name == "Toggle" then
		...
		Toggle.Name = Elisium:_RandomString()
		Toggle.Parent = LinkValues
		Toggle.BackgroundColor3 = Elisium.Colors.DropColor
		...
		table.insert(Elisium.Elements.DropColor , {
			Element = Toggle,
		...
		UIStroke.Color = Elisium.Colors.HighStrokeColor
		UIStroke.Parent = Toggle

		table.insert(Elisium.Elements.HighStrokeColor,{
			Element = UIStroke,
			Property = "Color"
		})

		ToggleValue.Name = Elisium:_RandomString()
		ToggleValue.Parent = Toggle
		ToggleValue.AnchorPoint = Vector2.new(0.5, 0.5)
		ToggleValue.BackgroundColor3 = Elisium.Colors.SwitchColor
		...
		local ToggleElement = function(bool,noChange)
			...
			if bool then
				...
				Elisium:_Animation(ToggleValue,rep.Tween,{
					Position = UDim2.new(0.75, 0, 0.5, 0)
				})

				Elisium:_Animation(Toggle,rep.Tween,{
					BackgroundColor3 = Elisium.Colors.Toggle
				})
			else
				...
				Elisium:_Animation(Toggle,rep.Tween,{
					BackgroundColor3 = Elisium.Colors.DropColor
				})
			end;
		end;

		local Input = Elisium:_Input(Toggle);

		Elisium:_Hover(Input , function()
			if not Default then
				Elisium:_Animation(ToggleValue,rep.Tween,{
		...
		local ToggleUI = function(bool)
			if bool then
				...
				Elisium:_Animation(UIStroke,rep.Tween,{
					Transparency = 0
				})
			else
	...
	elseif Name == "ColorPicker" then
		...
		ColorPicker.Name = Elisium:_RandomString()
		...
		ColorFrame.Name = Elisium:_RandomString()
		...
		Signal:Connect(function(bool)
			if bool then
				Elisium:_Animation(ColorFrame,TweenInfo.new(0.15),{
					BackgroundTransparency = 0,
				})

				Elisium:_Animation(UIStroke,TweenInfo.new(0.15),{
					Transparency = 0,
				})
			else
				Elisium:_Animation(ColorFrame,TweenInfo.new(0.15),{
		...
		Elisium:_Hover(ColorPicker, function()
			if Signal:GetValue() then
				Elisium:_Animation(UIScale,TweenInfo.new(0.35),{
					Scale = 1.2
				})
			end;
		end , function()
	...
	elseif Name == "Keybind" then
		...
		Keybind.Name = Elisium:_RandomString()
		Keybind.Parent = LinkValues
		Keybind.BackgroundColor3 = Elisium.Colors.DropColor
		...
		TextLabel.TextColor3 = Elisium.Colors.SwitchColor
		TextLabel.TextSize = 12.000
		TextLabel.TextTransparency = 0.200

		table.insert(Elisium.Elements.SwitchColor,{
			Element = TextLabel,
			Property = "TextColor3"
		});
		...
		local Update = function()
			...
			Elisium:_Animation(Keybind,TweenInfo.new(0.1),{
		...
		local ToggleUI = function(bool)
			if bool then
				Elisium:_Animation(Keybind,rep.Tween,{
				...
				Elisium:_Animation(TextLabel,rep.Tween,{
	...
	elseif Name == "Helper" then
		...
		InfoButton.Name = Elisium:_RandomString()
		...
		BlockText.Name = Elisium:_RandomString()
		BlockText.Parent = InfoButton
		BlockText.AnchorPoint = Vector2.new(0, 0)
		BlockText.BackgroundColor3 = Elisium.Colors.BlockColor

		table.insert(Elisium.Elements.BlockColor , {
			Element = BlockText,
			Property = "BackgroundColor3"
		});
		...
		BlockText.Text = " "
		BlockText.TextColor3 = Elisium.Colors.SwitchColor
		...
		UIStroke.Color = Elisium.Colors.StrokeColor
		UIStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
		UIStroke.Parent = BlockText

		table.insert(Elisium.Elements.StrokeColor,{
			Element = UIStroke,
			Property = "Color"
		});
		...
		Signal:Connect(function(bool)
			if bool then
				Elisium:_Animation(InfoButton,TweenInfo.new(0.15),{
		...
		Elisium:_Hover(InfoButton, function()
			if Signal:GetValue() then
				Elisium:_Animation(InfoButton,TweenInfo.new(0.15),{
	...
	elseif Name == "Option" then
		...
		OptionButton.Name = Elisium:_RandomString()
		...
		Signal:Connect(function(bool)
			if bool then
				Elisium:_Animation(OptionButton,TweenInfo.new(0.15),{
					ImageTransparency = 0.500
				})
			else
				Elisium:_Animation(OptionButton,TweenInfo.new(0.15),{
		...
		Elisium:_Hover(OptionButton, function()
			if Signal:GetValue() then
				Elisium:_Animation(OptionButton,TweenInfo.new(0.15),{
					ImageTransparency = 0.1
				})
...
function Elisium:_CreateBlock(Signal)
	...
	if Elisium:_IsMobile() then
		Elisium:_AddDragBlacklist(GlobalBlock);
	end;

	GlobalBlock.Name = Elisium:_RandomString()
	...
	LinkValues.Name = Elisium:_RandomString()
	...
	BlockLine.Name = Elisium:_RandomString()
	BlockLine.Parent = GlobalBlock
	BlockLine.AnchorPoint = Vector2.new(0.5, 1)
	BlockLine.BackgroundColor3 = Elisium.Colors.LineColor
	...
	table.insert(Elisium.Elements.LineColor,{
		Element = BlockLine,
		Property = "BackgroundColor3"
	});
	...
	function rep:SetTextColor(Color)
		local oldIndex = table.find(Elisium.Elements.SwitchColor , BlockText);

		table.remove(Elisium.Elements.SwitchColor , oldIndex);

		BlockText.TextColor3 = Color;

		table.insert(Elisium.Elements.Risky , {
			Element = BlockText,
			Property = 'TextColor3'
		});
	...
	function rep:SetTransparency(num)
		...
		Elisium:_Animation(BlockText,TweenInfo.new(0.3),{
	...
	function rep:SetVisible(bool)
		if bool then
			Elisium:_Animation(BlockText,rep.Tween,{
				TextTransparency = rep.TextTransparency
			});

			Elisium:_Animation(BlockLine,rep.Tween,{
	...
	function rep:AddLink(Name , Default)
		return Elisium:_AddLinkValue(Name , Default , GlobalBlock , LinkValues , rep , Signal);
	end;

	return rep;
end;

function Elisium:_AddColorPickerPanel(Button: ImageButton , Callback: (Color: Color3) -> any?)
	local Window = Elisium:_GetWindowFromElement(Button);
	...
	ColorPickerWindow.Name = Elisium:_RandomString()
	ColorPickerWindow.Parent = Window
	ColorPickerWindow.BackgroundColor3 = Elisium.Colors.BlockBackground
	...
	table.insert(Elisium.Elements.BlockBackground,{
		Element = ColorPickerWindow,
		Property = "BackgroundColor3"
	});

	Elisium:_AddDragBlacklist(ColorPickerWindow)
	...
	ColorPickBox.Name = Elisium:_RandomString()
	...
	MouseMovement.Name = Elisium:_RandomString()
	...
	ColorRedGreenBlue.Name = Elisium:_RandomString()
	...
	ColorRGBSlide.Name = Elisium:_RandomString()
	...
	Left.Name = Elisium:_RandomString()
	...
	Right.Name = Elisium:_RandomString()
	...
	ColorOpc.Name = Elisium:_RandomString()
	...
	ColorOptSlide.Name = Elisium:_RandomString()
	...
	Left_2.Name = Elisium:_RandomString()
	...
	Right_2.Name = Elisium:_RandomString()
	...
	HexFrame.Name = Elisium:_RandomString()
	HexFrame.Parent = ColorPickerWindow
	HexFrame.AnchorPoint = Vector2.new(0.5, 1)
	HexFrame.BackgroundColor3 = Elisium.Colors.BlockColor
	...
	UIStroke_8.Color = Elisium.Colors.HighStrokeColor
	...
	Elisium:_AddPropertyEvent(ColorPickerWindow,function(v)
	...
	local ToggleUI = function(bool)
		...
		if bool then
			...
			Elisium:_Animation(ColorPickerWindow,Tween2,{
				BackgroundTransparency = 0,
				Size = UDim2.new(0, 175, 0, 200)
			});

			Elisium:_Animation(ColorPickerWindow,Tween,{
				Position = MainPosition,
			});

			Elisium:_Animation(UIStroke_8,Tween,{
				Transparency = 0
			});

			Elisium:_Animation(UIStroke_7,Tween,{
				Transparency = 0.5
			});

			Elisium:_Animation(UIStroke_6,Tween,{
				Transparency = 0
			});

			Elisium:_Animation(UIStroke_5,Tween,{
				Transparency = 0
			});

			Elisium:_Animation(UIStroke_4,Tween,{
				Transparency = 0
			});

			Elisium:_Animation(UIStroke_3,Tween,{
				Transparency = 0
			});

			Elisium:_Animation(UIStroke_2,Tween,{
				Transparency = 0
			});

			Elisium:_Animation(UIStroke,Tween,{
				Transparency = 0
			});

			Elisium:_Animation(ColorPickBox,Tween,{
				BackgroundTransparency = 0,
				ImageTransparency = 0
			});

			Elisium:_Animation(MouseMovement,Tween,{
				ImageTransparency = 0
			});

			Elisium:_Animation(ColorOpc,Tween,{
				BackgroundTransparency = 0
			});

			Elisium:_Animation(TransparentImage,Tween,{
				ImageTransparency = 0
			});

			Elisium:_Animation(Left,Tween,{
				BackgroundTransparency = 0
			});

			Elisium:_Animation(Left_2,Tween,{
				BackgroundTransparency = 0
			});

			Elisium:_Animation(Right,Tween,{
				BackgroundTransparency = 0
			});

			Elisium:_Animation(Right_2,Tween,{
				BackgroundTransparency = 0
			});

			Elisium:_Animation(ColorRedGreenBlue,Tween,{
				BackgroundTransparency = 0
			});

			Elisium:_Animation(HexFrame,Tween,{
				BackgroundTransparency = 0
			});

			Elisium:_Animation(TextLabel,Tween,{
				TextTransparency = 0
			});
		else
			Elisium:_Animation(UIStroke_8,Tween,{
	...
	function Args:Update()
		...
		Elisium:_Animation(ColorPickBox,TweenInfo.new(0.2),{
			BackgroundColor3 = Color3.fromHSV(H , 1 , 1)
		});

		Elisium:_Animation(ColorOpc,TweenInfo.new(0.2),{
			BackgroundColor3 = RealColor
		});

		Elisium:_Animation(MouseMovement,TweenInfo.new(0.2),{
			Position = UDim2.fromScale(S , 1 - V)
		});

		Elisium:_Animation(ColorOptSlide,TweenInfo.new(0.2),{
			Position = UDim2.new(Transparency ,0 , 0.5 ,0)
		});

		Elisium:_Animation(ColorRGBSlide,TweenInfo.new(0.2),{
	...
	UserInputService.InputBegan:Connect(function(Input)
		if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
			if not Elisium:_IsMouseOverFrame(ColorPickerWindow) then
				ToggleUI(false);
			end;
		end;
	end)
...
function Elisium:_LoadOption(Value , TabSignal)
	...
	function Args:AddKeybind(Config: MiniKeybind)
		...
		Elisium:_Input(Keybind.Root,function()
			if IsBinding then
				return;
			end;
		...
		if Config.Flag then
			Elisium.Flags[Config.Flag] = Args;
		end;
	...
	function Args:AddHelper(Config: Helper)
		...
		local Update = function()
			...
			Elisium:_Animation(Helper.Text , TweenInfo.new(0.15), {
		...
		local Release = function()
			...
			Elisium:_Animation(Helper.UIStroke,TweenInfo.new(0.15),{
				Transparency = 1
			});
		end;
		...
		Elisium:_Hover(Button,  Hold, Release);
	...
	function Args:AddColorPicker(Config: MiniColorPicker)
		...
		local Button = Elisium:_Input(ColorPicker);

		local ColorPicker = Elisium:_AddColorPickerPanel(Button,function(color,opc)
	...
	function Args:AddOption()
		...
		local Signal = Elisium.__SIGNAL(false);
		...
		local ToggleUI = function(bool)
			...
			if bool then
				...
				Elisium:_Animation(ExtractElement , Tween , {
					Position = MainPosition,
					BackgroundTransparency = 0,
		...
		Elisium:_AddPropertyEvent(ExtractElement,function(bool)
			ExtractElement.Visible = bool;
		end);

		Elisium:_AddDragBlacklist(ExtractElement);

		ExtractElement.Name = Elisium:_RandomString()
		ExtractElement.Parent = Window
		ExtractElement.BackgroundColor3 = Elisium.Colors.BlockBackground
		...
		Elements.Name = Elisium:_RandomString()
		...
		UserInputService.InputBegan:Connect(function(Input)
			if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
				if Toggl and not Elisium:_IsMouseOverFrame(ExtractElement) and not Elisium:_IsMouseOverFrame(Element) then
					ToggleUI(false);
				end;
			end
		end)		

		return Elisium:_LoadElement(Elements , true , Signal)
	end;

	return Args;
end;

function Elisium:_LoadDropdown(BaseParent: TextButton , Callback: () -> any)
	local Window = Elisium:_GetWindowFromElement(BaseParent);
	...
	local UIListLayout = Instance.new("UIListLayout")
	local ToggleDb = Elisium.__SIGNAL(false);
	local EventOut = Elisium.__SIGNAL(0);

	DropdownWindow.Name = Elisium:_RandomString()
	DropdownWindow.Parent = Window
	DropdownWindow.BackgroundColor3 = Elisium.Colors.BlockBackground
	...
	Elisium:_AddDragBlacklist(DropdownWindow);
	Elisium:_AddPropertyEvent(DropdownWindow,function(v)
	...
	local DrawButton = function()
		...
		DropdownItem.Name = Elisium:_RandomString()
	...
	local UpdateDropdown = function()
		...
		for i,v in next , Values do
			...
			table.insert(__signals , ToggleDb:Connect(function(bool)
				if bool then
					Elisium:_Animation(bth.BlockText,TweenInfo.new(0.2),{
						TextTransparency = ((IsDefault(v) or MatchDefault(v,DataFrame)) and 0) or 0.5
					});

					Elisium:_Animation(bth.BlockLine,TweenInfo.new(0.2),{
						BackgroundTransparency = 0
					});
				else
					Elisium:_Animation(bth.BlockText,TweenInfo.new(0.2),{
			...
			if IsMulti then
				...
				Elisium:_Input(bth.DropdownItem,function()
				...
				Elisium:_Input(bth.DropdownItem,function()
					if DataFrame then
						Elisium:_Animation(DataFrame.BlockText,TweenInfo.new(0.2),{
							TextTransparency = ((IsDefault(v) or MatchDefault(v,DataFrame)) and 0) or 0.5
						});
					end;
	...
	UserInputService.InputBegan:Connect(function(Input)
		if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
			if not Elisium:_IsMouseOverFrame(DropdownWindow) then
				ToggleUI(false);
			end;
		end;
	end);
...
function Elisium:_LoadElement(Parent: Frame , EnabledLine: boolean , Signal)
	...
	function Args:AddToggle(Config : Toggle)
		...
		local Block = Elisium:_CreateBlock(Signal);
		...
		if Config.Risky then
			Block:SetTextColor(Elisium.Colors.Risky);
		end;
		...
		Args.Link = Elisium:_LoadOption(Block);
	...
	function Args:AddButton(Config: Button)
		...
		if Elisium:_IsMobile() then
			Elisium:_AddDragBlacklist(Button);
		end;

		Button.Name = Elisium:_RandomString()
		...
		Frame.BackgroundColor3 = Elisium.Colors.Highlight
		...
		table.insert(Elisium.Elements.Highlight,{
			Element = Frame,
			Property = "BackgroundColor3"
		});
		...
		Elisium:_Hover(Frame,function()
			if Signal:GetValue() then
				Elisium:_Animation(Frame,TweenInfo.new(0.2),{
					BackgroundTransparency = 0
				})
			end;
		end,function()
		...
		Args.Signal = Signal:Connect(function(bool)
			if bool then
				Elisium:_Animation(BlockLine, TweenInfo.new(0.35),{
					BackgroundTransparency = 0.500
				});

				Elisium:_Animation(Frame, TweenInfo.new(0.35),{
					BackgroundTransparency = 0.1
				});

				Elisium:_Animation(UIStroke, TweenInfo.new(0.35),{
					Transparency = 0
				});

				Elisium:_Animation(TextLabel, TweenInfo.new(0.35),{
	...
	function Args:AddSlider(Config: Slider)
		...
		Elisium:_AddDragBlacklist(Slider);

		Slider.Name = Elisium:_RandomString()
		...
		SliderBar.Name = Elisium:_RandomString()
		SliderBar.Parent = Slider
		SliderBar.AnchorPoint = Vector2.new(0.5, 1)
		SliderBar.BackgroundColor3 = Elisium.Colors.DropColor
		...
		SliderInput.Name = Elisium:_RandomString()
		SliderInput.Parent = SliderBar
		SliderInput.AnchorPoint = Vector2.new(0, 0.5)
		SliderInput.BackgroundColor3 = Elisium.Colors.Highlight
		...
		Frame.BackgroundColor3 = Elisium.Colors.SwitchColor
		Frame.BorderColor3 = Color3.fromRGB(0, 0, 0)
		Frame.BorderSizePixel = 0
		...
		ValueText.Name = Elisium:_RandomString()
		...
		ValueText.TextColor3 = Elisium.Colors.SwitchColor
		...
		Elisium:_Hover(SliderBar,function()
			if Signal:GetValue() then
				Elisium:_Animation(ValueText,TweenInfo.new(0.2),{
		...
		local Update = function(Input)
			...
			local Value = Elisium:_Rounding(Main,Config.Round);
			...
			SliderBar.InputEnded:Connect(function(Input)
				if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
					if UserInputService.TouchEnabled then
						if not Elisium:_IsMouseOverFrame(SliderBar) then
							IsHold = false
						end;
					else
						IsHold = false
		...
		function Args:SetValue(Value)
			...
			Elisium:_Animation(SliderInput, TweenInfo.new(0.35),{
		...
		Args.Signal = Signal:Connect(function(bool)
			if bool then
				...
				Elisium:_Animation(ValueText,Tween,{
					TextTransparency = 0.750
				})

				Elisium:_Animation(Frame,Tween,{
					BackgroundTransparency = 0
				})

				Elisium:_Animation(SliderInput,Tween,{
					BackgroundTransparency = 0
				})
				...
				Elisium:_Animation(SliderBar,Tween,{
					BackgroundTransparency = 0
				})

				Elisium:_Animation(BlockLine,Tween,{
					BackgroundTransparency = 0.5
				})

				Elisium:_Animation(BlockText,Tween,{
	...
	function Args:AddParagraph(Config: Paragraph) -- request by Neptune
		...
		if Elisium:_IsMobile() then
			Elisium:_AddDragBlacklist(Paragraph);
		end;

		Paragraph.Name = Elisium:_RandomString()
		...
		DescriptionText.Name = Elisium:_RandomString()
		...
		DescriptionText.TextColor3 = Elisium.Colors.SwitchColor
		...
		local UpdateScale = function()
			...
			if not DescriptionText.Text:byte() then
				...
				Elisium:_Animation(Paragraph,TweenInfo.new(0.15),{
		...
		Args.Signal = Signal:Connect(function(bool)
			if bool then
				Elisium:_Animation(BlockText,TweenInfo.new(0.2),{
					TextTransparency = 0.300
				});

				Elisium:_Animation(DescriptionText,TweenInfo.new(0.2),{
					TextTransparency = 0.500
				});

				Elisium:_Animation(BlockLine,TweenInfo.new(0.2),{
					BackgroundTransparency = 0.500
				});
			else
				Elisium:_Animation(BlockText,TweenInfo.new(0.2),{
	...
	function Args:AddTextBox(Config: TextBoxConfig)
		...
		if Elisium:_IsMobile() then
			Elisium:_AddDragBlacklist(TextBox);
		end;

		TextBox.Name = Elisium:_RandomString()
		...
		LinkValues.AnchorPoint = Vector2.new(1, 0.540000021)
		LinkValues.BackgroundColor3 = Elisium.Colors.DropColor
		LinkValues.BorderColor3 = Color3.fromRGB(0, 0, 0)
		LinkValues.BorderSizePixel = 0
		LinkValues.Position = UDim2.new(1, -12, 0.5, 0)
		...
		TextBox_2.TextColor3 = Elisium.Colors.SwitchColor
		...
		local Update = function()
			...
			Elisium:_Animation(LinkValues,TweenInfo.new(0.25),{
		...
		Args.Signal = Signal:Connect(function(bool)
			if bool then
				...
				Elisium:_Animation(LinkValues,TweenInfo.new(0.2),{
					BackgroundTransparency = 0
				});
	...
	function Args:AddDropdown(Config : Dropdown)
		...
		Dropdown.Name = Elisium:_RandomString()
		...
		ValueItems.Name = Elisium:_RandomString()
		ValueItems.Parent = Dropdown
		ValueItems.AnchorPoint = Vector2.new(0.5, 1)
		ValueItems.BackgroundColor3 = Elisium.Colors.DropColor
		...
		MainButton.Name = Elisium:_RandomString()
		...
		Elisium:_Hover(ValueItems,function()
			Elisium:_Animation(ValueItems,TweenInfo.new(0.3),{
				BackgroundColor3 = Elisium.Colors.MouseEnter
			});
		end,function()
			Elisium:_Animation(ValueItems,TweenInfo.new(0.3),{
				BackgroundColor3 = Elisium.Colors.DropColor
		...
		local repi;
		local Button = Elisium:_Input(ValueItems);

		repi = Elisium:_LoadDropdown(Button,function(value)
			Config.Default = value;
		...
		repi.EventOut:Connect(function(v)
			if v then
				Elisium:_Animation(MainButton,TweenInfo.new(0.2),{
					Rotation = -180
				})
			else
				Elisium:_Animation(MainButton,TweenInfo.new(0.2),{
		...
		Args.Signal = Signal:Connect(function(bool)
			if bool then
				...
				Elisium:_Animation(ValueItems,TweenInfo.new(0.2),{
					BackgroundTransparency = 0
				});
				...
				Elisium:_Animation(ValueText,TweenInfo.new(0.32),{
					TextTransparency = 0
				});
		...
		Args.Link = Elisium:_LoadOption({
			AddLink = function(self ,Name , Default)
				return Elisium:_AddLinkValue(Name , Default , LinkValues , LinkValues , {
...
function Elisium:GetTheme()
	return Elisium.Colors;
end;

function Elisium:SetTheme(name)
	...
	Elisium:RefreshCurrentColor()
end;

function Elisium:RefreshCurrentColor()
	for i,v in next , Elisium.Elements.Highlight do
		if v.Element and v.Property then
			v.Element[v.Property] = Elisium.Colors.Highlight;
		end;
	end;

	for i,v in next , Elisium.Elements do
		if v.Element and v.Property and v.Element:GetAttribute('Enabled') then
			v.Element[v.Property] = Elisium.Colors.Highlight;
		end;
	end;

	for i,v in next , Elisium.Elements.Risky do
		if v.Element and v.Property then
			v.Element[v.Property] = Elisium.Colors.Risky;
		end;
	end;

	for i,v in next , Elisium.Elements.BlockColor do
		if v.Element and v.Property then
			v.Element[v.Property] = Elisium.Colors.BlockColor;
		end;
	end;

	for i,v in next , Elisium.Elements.BGDBColor do
		if v.Element and v.Property then
			v.Element[v.Property] = Elisium.Colors.BGDBColor;
		end;
	end;

	for i,v in next , Elisium.Elements.StrokeColor do
		if v.Element and v.Property then
			v.Element[v.Property] = Elisium.Colors.StrokeColor;
		end;
	end;

	for i,v in next , Elisium.Elements.SwitchColor do
		if v.Element and v.Property and v.Element[v.Property] ~= Elisium.Colors.MouseEnter then
			v.Element[v.Property] = Elisium.Colors.SwitchColor;
		end;
	end;

	for i,v in next , Elisium.Elements.BlockBackground do
		if v.Element and v.Property and v.Element[v.Property] then
			v.Element[v.Property] = Elisium.Colors.BlockBackground;
		end;
	end;

	for i,v in next , Elisium.Elements.DropColor do
		if v.Element and v.Property then
			v.Element[v.Property] = Elisium.Colors.DropColor;
		end;
	end;

	for i,v in next , Elisium.Elements.LineColor do
		if v.Element and v.Property then
			v.Element[v.Property] = Elisium.Colors.LineColor;
		end;
	end;

	for i,v in next , Elisium.Elements.HighStrokeColor do
		if v.Element and v.Property then
			v.Element[v.Property] = Elisium.Colors.HighStrokeColor;
		end;
	end;
end;

function Elisium:ChangeHighlightColor(NewColor: Color3)
	local H,S,V = NewColor:ToHSV();

	Elisium.Colors.Highlight = NewColor;
	Elisium.Colors.Toggle = Color3.fromHSV(H,S,V - 0.2);
...
function Elisium.new(Config : Window)

	if not Config.Scale then
		if Elisium:_IsMobile() then
			Config.Scale = Elisium.Scale.Mobile;
		else
			Config.Scale = Elisium.Scale.Window;
		end;
	end;
	...
	Config = Elisium.__CONFIG(Config , {
		...
		Keybind = "Insert",
		Logo = Elisium.Logo;
		Scale = Elisium.Scale.Window,
		TextSize = 15
	});

	local TabHover = Elisium.__SIGNAL(false);
	local WindowOpen = Elisium.__SIGNAL(true);
	local WindowArgs = {
		SelectedTab = nil,
		Tabs = {},
		LastTab = nil,
	...
	local EliSium = Instance.new("ScreenGui")
	...
	EliSium.Name = "u?name=alwayswin_"..Elisium:_RandomString();
	EliSium.Parent = CoreGui;
	EliSium.ResetOnSpawn = false
	EliSium.IgnoreGuiInset = true;
	EliSium.ZIndexBehavior = Enum.ZIndexBehavior.Global;

	table.insert(Elisium.Windows , EliSium);

	MainFrame.Active = true;
	MainFrame.Name = Elisium:_RandomString()
	MainFrame.Parent = EliSium
	MainFrame.AnchorPoint = Vector2.new(0.5, 0.5)
	MainFrame.BackgroundColor3 = Elisium.Colors.BGDBColor

	table.insert(Elisium.Elements.BGDBColor,{
		Element = MainFrame,
		Property = 'BackgroundColor3'
	});
	...
	MainFrame.Size = Elisium.Scale.Window
	MainFrame.ZIndex = 4
	...
	Elisium:_Animation(MainFrame,TweenInfo.new(1,Enum.EasingStyle.Quint,Enum.EasingDirection.InOut),{
		Size = Config.Scale
	});
	...
	TabFrame.Name = Elisium:_RandomString()
	TabFrame.Parent = MainFrame
	TabFrame.AnchorPoint = Vector2.new(1, 0)
	TabFrame.BackgroundColor3 = Elisium.Colors.BGDBColor
	...
	LineFrame1.Name = Elisium:_RandomString()
	LineFrame1.Parent = TabFrame
	LineFrame1.AnchorPoint = Vector2.new(1, 0)
	LineFrame1.BackgroundColor3 = Elisium.Colors.BGDBColor
	...
	CompLogo.Name = Elisium:_RandomString()
	...
	WindowLabel.Name = Elisium:_RandomString()
	...
	WindowLabel.TextColor3 = Elisium.Colors.SwitchColor
	...
	TabButtons.Name = Elisium:_RandomString()
	...
	SelectionFrame.Name = Elisium:_RandomString()
	SelectionFrame.Parent = TabButtons
	SelectionFrame.AnchorPoint = Vector2.new(1, 0)
	SelectionFrame.BackgroundColor3 = Elisium.Colors.Highlight
	...
	TabButtonScrollingFrame.Name = Elisium:_RandomString()
	...
	Userinfo.Name = Elisium:_RandomString()
	...
	UserProfile.Name = Elisium:_RandomString()
	...
	UserText.Name = Elisium:_RandomString()
	...
	UserText.TextColor3 = Elisium.Colors.SwitchColor
	...
	ExpireText.Name = Elisium:_RandomString()
	...
	ExpireText.TextColor3 = Elisium.Colors.SwitchColor
	...
	TabMainFrame.Name = Elisium:_RandomString()
	...
	if Elisium:_IsMobile() then
		Elisium:_AddDragBlacklist(TabButtons);
	end;
	...
	WindowOpen:Connect(function(v)
		...
		if v then
			Elisium:_Animation(MainFrame,TweenInfo.new(0.2),{
				Size = Config.Scale
			})

			Elisium:_Animation(TabButtonScrollingFrame,TweenInfo.new(0.35),{
				Position = UDim2.new(0.5, 0, 0.5, 0)
			})

			Elisium:_Animation(CompLogo,TweenInfo.new(0.2),{
				ImageTransparency = 0
			})

			Elisium:_Animation(WindowLabel,TweenInfo.new(0.2),{
				TextTransparency = 0
			})

			Elisium:_Animation(UserProfile,TweenInfo.new(0.2),{
				ImageTransparency = 0
			})

			Elisium:_Animation(UserText,TweenInfo.new(0.2),{
				TextTransparency = 0
			})

			Elisium:_Animation(ExpireText,TweenInfo.new(0.2),{
			...
			Elisium:_Animation(LineFrame1,TweenInfo.new(0.3),{
				BackgroundTransparency = 0,
				Size = UDim2.new(0, 20, 1, 0)
			})

			Elisium:_Animation(TabFrame,TweenInfo.new(0.2),{
			...
			Elisium:_Animation(LineFrame1,TweenInfo.new(0.1),{
				BackgroundTransparency = 1,
			...
			Elisium:_Animation(TabFrame,TweenInfo.new(0.1),{
				BackgroundTransparency = 1
			})
		end;
	end);
	...
	TabHover:Connect(function(value)
		...
		if value then
			Elisium:_Animation(TabFrame , Style , {
				Size = UDim2.new(0, 185,1, 0)
			});

			Elisium:_Animation(WindowLabel , Style , {
				Position = UDim2.new(0, 60,0, 17),
				TextTransparency = 0
			});

			Elisium:_Animation(UserText , Style , {
				Position = UDim2.new(0, 55,0, 8),
				TextTransparency = 0.1
			});

			Elisium:_Animation(ExpireText , Style , {
	...
	function WindowArgs:DrawCategory(config : Category)
		...
		Category.Name = Elisium:_RandomString()
		...
		if Elisium:_IsMobile() then
			Elisium:_AddDragBlacklist(Category);
		end;

		CategoryText.Name = Elisium:_RandomString()
		...
		CategoryText.TextColor3 = Elisium.Colors.SwitchColor
		...
		TabHover:Connect(function(bool)
			if bool then
				Elisium:_Animation(CategoryText,Tween,{
					TextTransparency = 0.500
				});
	...
	function WindowArgs:DrawContainerTab(TabConfig : ContainerTab)
		TabConfig = Elisium.__CONFIG(TabConfig,{
			Name = "Tab",
			Icon = "eye",
		});

		local Tween = TweenInfo.new(0.35,Enum.EasingStyle.Quint);
		local TabOpenSignal = Elisium.__SIGNAL(false);
		...
		TabButton.Name = Elisium:_RandomString()
		...
		if Elisium:_IsMobile() then
			Elisium:_AddDragBlacklist(TabButton);
		end;

		Icon.Name = Elisium:_RandomString()
		Icon.Parent = TabButton
		Icon.AnchorPoint = Vector2.new(0, 0.5)
		Icon.BackgroundColor3 = Elisium.Colors.Highlight
		...
		Icon.ZIndex = 3
		Icon.Image = Elisium:_GetIcon(TabConfig.Icon);
		Icon.ImageColor3 = Elisium.Colors.Highlight
		...
		TabNameLabel.Name = Elisium:_RandomString()
		...
		TabNameLabel.TextColor3 = Elisium.Colors.SwitchColor
		...
		Highlight.Name = Elisium:_RandomString()
		...
		ContainerTab.Name = Elisium:_RandomString()
		...
		Top.Name = Elisium:_RandomString()
		...
		local TabOpen = function(bool)
			if bool then
				...
				Elisium:_Animation(Icon,Tween,{
					ImageTransparency = 0,
				});

				Elisium:_Animation(TabNameLabel,Tween,{
					TextTransparency = 0
				});

				Elisium:_Animation(Highlight,Tween,{
		...
		Elisium:_Hover(TabButton,function()
		...
		TabHover:Connect(function(bool)
			if bool then
				Elisium:_Animation(TabButton,Tween,{
				...
				Elisium:_Animation(UICorner,Tween,{
		...
		Elisium:_Input(TabButton,function()
		...
		function TabArgs:DrawTab(TabConfig : TabConfig) -- Internal Tab
			...
			local InternalSignal = Elisium.__SIGNAL(false);
			local Frame = Instance.new("Frame")
			local UICorner = Instance.new("UICorner")
			local UIStroke = Instance.new("UIStroke")
			local Highlight = Instance.new("Frame")
			...
			Frame.Parent = Top
			Frame.BackgroundColor3 = Elisium.Colors.BlockColor
			...
			Highlight.BackgroundColor3 = Elisium.Colors.Highlight
			...
			local ToggleUI = function(bool)
				...
				if bool then
					...
					Elisium:_Animation(Highlight,TweenInfo.new(0.2),{
						BackgroundTransparency = 0,
					...
					Elisium:_Animation(TextLabel,TweenInfo.new(0.2),{
						TextTransparency = 0
					})
				else
	...
	function WindowArgs:DrawConfig(Configuration : TabConfigManager , Internal)
		Configuration = Elisium.__CONFIG(Configuration,{
			Name = "Config",
			Icon = "folder",
			Config = nil
		});
		...
		Icon.Size = UDim2.new(0, 22, 0, 22)
		Icon.ZIndex = 3
		Icon.Image = Elisium:_GetIcon(Configuration.Icon);
		Icon.ImageColor3 = Elisium.Colors.Highlight
		...
		TabConfig.Name = Elisium:_RandomString()
		...
		ConfigList.Name = Elisium:_RandomString()
		ConfigList.Parent = TabConfig
		ConfigList.AnchorPoint = Vector2.new(0.5, 0)
		ConfigList.BackgroundColor3 = Elisium.Colors.BlockColor
		...
		Header.Name = Elisium:_RandomString()
		...
		SectionText.Name = Elisium:_RandomString()
		...
		SectionText.TextColor3 = Elisium.Colors.SwitchColor
		...
		SectionClose.Name = Elisium:_RandomString()
		...
		UIStroke_2.Color = Elisium.Colors.StrokeColor
		...
		Header_2.Name = Elisium:_RandomString()
		...
		SectionText_2.Name = Elisium:_RandomString()
		...
		SectionText_2.TextColor3 = Elisium.Colors.SwitchColor
		...
		SectionClose_2.Name = Elisium:_RandomString()
		...
		UIStroke_3.Color = Elisium.Colors.StrokeColor
		...
		TextBox.Text = ""
		TextBox.TextColor3 = Elisium.Colors.SwitchColor
		...
		Button.BackgroundColor3 = Elisium.Colors.SwitchColor
		Button.BackgroundTransparency = 1.000
		Button.BorderColor3 = Color3.fromRGB(0, 0, 0)
		Button.BorderSizePixel = 0
		...
		Frame_2.BackgroundColor3 = Elisium.Colors.Highlight
		...
		UIStroke_4.Color = Elisium.Colors.StrokeColor
		...
		local TabOpen = function(bool)
			if bool then
				...
				Elisium:_Animation(ConfigList,Tween,{
					BackgroundTransparency = 0,
				});

				Elisium:_Animation(AddConfig,Tween,{
					BackgroundTransparency = 0,
				});
				...
				Elisium:_Animation(SectionText,Tween,{
					TextTransparency = 0.5
				});
				...
				Elisium:_Animation(Frame_2,Tween,{
					BackgroundTransparency = 0.1,
				});
				...
				Elisium:_Animation(SectionText_2,Tween,{
					TextTransparency = 0.5
				});

				Elisium:_Animation(TextBox,Tween,{
					TextTransparency = 0
				});

				Elisium:_Animation(SectionClose,Tween,{
					ImageTransparency = 0.5,
				});

				Elisium:_Animation(SectionClose_2,Tween,{
					ImageTransparency = 0.5,
				});
			else
		...
		function TabArgs:_DrawConfig()
			...
			DelButton.Name = Elisium:_RandomString()
			...
			ConfigBlock.Name = Elisium:_RandomString()
			...
			if Elisium:_IsMobile() then
				Elisium:_AddDragBlacklist(ConfigBlock);
			end;

			ConfigText.Name = Elisium:_RandomString()
			...
			ConfigText.TextColor3 = Elisium.Colors.SwitchColor
			...
			SaveButton.Name = Elisium:_RandomString()
			...
			LoadButton.Name = Elisium:_RandomString()
			...
			TextLabel_2.TextColor3 = Elisium.Colors.SwitchColor
			...
			Icon_2.Name = Elisium:_RandomString()
			...
			AuthorText.Name = Elisium:_RandomString()
			...
			AuthorText.TextColor3 = Elisium.Colors.SwitchColor
			...
			function ConfigButton:SetInfo(Author , ConfigName)
				local R,G,B = tostring(math.floor(Elisium.Colors.Highlight.R * 255)) , tostring(math.floor(Elisium.Colors.Highlight.G * 255)) , tostring(math.floor(Elisium.Colors.Highlight.B * 255));
			...
			function ConfigButton:Toggle(v)
				if v then
					Elisium:_Animation(ConfigBlock,Tween,{
						BackgroundTransparency = 0
					});

					Elisium:_Animation(LinkValues,Tween,{
						Position = UDim2.new(1, -12, 0.5, 0)
					});

					Elisium:_Animation(ConfigText,Tween,{
						TextTransparency = 0.3,
						Position = UDim2.new(0, 12, 0.5, 0)
					});
					...
					Elisium:_Animation(AuthorText,Tween,{
						TextTransparency = 0.5,
						Position = UDim2.new(0,AuthorText:GetAttribute('SPC'), 0.5, 0)
					});

					Elisium:_Animation(Icon_2,Tween,{
						ImageTransparency = 0
					});
					...
					Elisium:_Animation(TextLabel_2,Tween,{
						TextStrokeTransparency = 0.900,
						TextTransparency = 0
					});
				else
			...
			Elisium:_Input(LoadButton,function()
				task.spawn(ConfigButton.OnLoad);
			end);

			Elisium:_Input(SaveButton,function()
		...
		function TabArgs:Init()
			...
			Elisium:_Input(Button,function()
				if TextBox.Text:byte() then
					WindowArgs.Notify.new({
						Title = "Configs",
						Icon = Elisium:_GetIcon(Config.Logo),
	...
	function WindowArgs:DrawTab(TabConfig : TabConfig , Internal)
		...
		if Internal then
			...
			TabContent.Name = Elisium:_RandomString()
			...
			if Elisium:_IsMobile() then
				Elisium:_AddDragBlacklist(Left);
				Elisium:_AddDragBlacklist(Right);
			end;
		...
		function TabArgs:DrawSection(config: Section)
			...
			Section.Name = Elisium:_RandomString()
			...
			Section.BackgroundColor3 = Elisium.Colors.BlockColor
			...
			if Elisium:_IsMobile() then
				Elisium:_AddDragBlacklist(Section);
			end;
			...
			local refresh = function(Upvalue)
				...
				if IsOpen then
					if Section:GetAttribute('Height') then
						Elisium:_Animation(Section,TweenInfo.new(0.4,Enum.EasingStyle.Quint),{
			...
			TabOpenSignal:Connect(function(bool)
				if bool then
					Elisium:_Animation(Section,TweenInfo.new(0.21),{
						BackgroundTransparency = 0
					})

					Elisium:_Animation(SectionText,TweenInfo.new(0.21),{
						TextTransparency = 0.500
					})

					Elisium:_Animation(SectionClose,TweenInfo.new(0.21),{
						ImageTransparency = 0.500
					})
				else
					Elisium:_Animation(Section,TweenInfo.new(0.21),{
			...
			Elisium:_Input(Header,function()
				IsOpen = not IsOpen;

				if IsOpen then
					Elisium:_Animation(SectionClose,TweenInfo.new(0.35),{
						Rotation = 0
					});
				else
					Elisium:_Animation(SectionClose,TweenInfo.new(0.35),{
			...
			return Elisium:_LoadElement(Section , true , TabOpenSignal)
		end;
	...
	do
		...
		CloseWindow.Name = Elisium:_RandomString()
		CloseWindow.Parent = EliSium
		CloseWindow.AnchorPoint = Vector2.new(1, 0)
		CloseWindow.BackgroundColor3 = Elisium.Colors.BGDBColor
		...
		local ToggleCloseUI = function(v)
			...
			if v then
				...
				Elisium:_Animation(CloseWindow,TweenInfo.new(0.2),{
					Size = UDim2.new(0, 45, 0, 23),
					BackgroundTransparency = 0.025
				})

				Elisium:_Animation(ImageLabel,TweenInfo.new(0.2),{
		...
		function WindowArgs:Watermark()
			...
			Watermark.Name = Elisium:_RandomString()
			Watermark.Parent = EliSium
			Watermark.AnchorPoint = Vector2.new(1, 0)
			Watermark.BackgroundColor3 = Elisium.Colors.BGDBColor
			...
			ImageLabel:GetPropertyChangedSignal('ClipsDescendants'):Connect(function()
				if ImageLabel.ClipsDescendants then
					Elisium:_Animation(Watermark , TweenInfo.new(0.2),{
			...
			Logo.Name = Elisium:_RandomString()
			Logo.Parent = Watermark
			Logo.AnchorPoint = Vector2.new(1, 0.5)
			Logo.BackgroundColor3 = Elisium.Colors.BGDBColor
			...
			WaternarkList.Name = Elisium:_RandomString()
			...
			BackFrame.Name = Elisium:_RandomString()
			...
			Elisium:_Blur(BackFrame,WindowOpen);

			UIListLayout:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(function()
				Elisium:_Animation(Watermark,TweenInfo.new(0.4),{
			...
			function Args:AddText(Watermark : Watermark)
				Watermark = Elisium.__CONFIG(Watermark, {
					Text = "Watermark",
					Icon = "info"
				});
				...
				Icon.SizeConstraint = Enum.SizeConstraint.RelativeYY
				Icon.ZIndex = 156
				Icon.Image = Elisium:_GetIcon(Watermark.Icon);
		...
		local Button = Elisium:_Input(CloseWindow,function()
			WindowArgs:_ToggleUI()
		end)

		if not Elisium:_IsMobile() then

			Elisium:_Hover(Button,function()
	...
	WindowArgs.LOOP_THREAD = task.spawn(function()
		...
		BlurElement.Name = Elisium:_RandomString()
		...
		Elisium:_Blur(BlurElement , WindowOpen);

		local MovementFrame = Instance.new("Frame")

		MovementFrame.Name = Elisium:_RandomString()
		...
		Elisium:Drag(MovementFrame,MainFrame,0.1)
		...
		while true do task.wait(0.01);
			...
			if WindowArgs.SelectedTab and WindowArgs.IsOpen then
				...
				if vili < 0 or vili > TabButtons.AbsoluteSize.Y then
					Elisium:_Animation(SelectionFrame , TweenInfo.new(0.1) , {
						BackgroundTransparency = 1
					});
				else
					if math.abs(distance) <= 10 then
						...
						Elisium:_Animation(SelectionFrame , TweenInfo.new(0.15) , {
							BackgroundTransparency = 0,
	...
	Elisium:_Hover(TabFrame , function()
...
function Elisium:GetTimeNow(Time)
	Time = Time or tick();
...
function Elisium:GetConfig(Type: string)
	local ConfigFlags = {};

	for i,v in next , Elisium.Flags do
...
function Elisium:_Path(...)
	local args = {...};

	return table.concat(args, "/");
end;

function Elisium:ConfigManager(ConfigManager: ConfigManager) : ConfigFunctions
	ConfigManager = Elisium.__CONFIG(ConfigManager , {
		Directory = "Elisium",
		Config = "Software"
	});
	...
	if not isfolder(Elisium:_Path(ConfigManager.Directory , ConfigManager.Config)) then
		makefolder(Elisium:_Path(ConfigManager.Directory , ConfigManager.Config));
	end;

	local Args = {
		Directory = Elisium:_Path(ConfigManager.Directory , ConfigManager.Config);
		EnableNotify = false,
	};

	local notify = Elisium.newNotify();

	function Args:WriteConfig(Config: WriteConfig)


		Config = Elisium.__CONFIG(Config , {
			Name = Elisium:_RandomString(),
			Author = LocalPlayer.Name,
		});

		local Flags = Elisium:GetConfig("MK");

		Flags["__INFORMATION"] = {
			Type = "Information",
			Author = Config.Author,
			Name = Config.Name,
			CreatedDate = Elisium:GetDate()
		};

		if Args.EnableNotify then
			notify.new({
				Title = "Configs",
				Icon = Elisium:_GetIcon('settings'),
				Content = "Create config \""..Config.Name.."\""
			})
		end

		writefile(Elisium:_Path(Args.Directory , Config.Name) , HttpService:JSONEncode(Flags));
	end;

	function Args:LoadConfigFromString(str: string)
		local decoded = HttpService:JSONDecode(str);

		local Flags = Elisium:GetConfig("KV");
	...
	function Args:GetCurrentConfig()
		return Elisium:GetConfig("MK")
	end;

	function Args:ReadInfo(ConfigName: string)
		local _path = Elisium:_Path(Args.Directory , ConfigName);
...
function Elisium:Loader(IconId,Duration)
	local EliSium = Instance.new("ScreenGui")

	EliSium.Name = Elisium:_RandomString()
	EliSium.Parent = CoreGui
	EliSium.Enabled = true
	EliSium.ResetOnSpawn = false
	EliSium.IgnoreGuiInset = true
	EliSium.ZIndexBehavior = Enum.ZIndexBehavior.Global
	...
	Loader.Name = Elisium:_RandomString()
	Loader.Parent = EliSium
	...
	Vignette.Name = Elisium:_RandomString()
	...
	Vignette.ImageColor3 = Elisium.Colors.Highlight
	Vignette.ImageTransparency = 1
	Vignette.AnchorPoint = Vector2.new(0.5,0.5)
	Vignette.Position = UDim2.fromScale(0.5,0.5)

	Elisium:_Animation(Loader,TweenInfo.new(0.55,Enum.EasingStyle.Quint),{
		BackgroundTransparency = 0.5
	});
	...
	task.delay(0.5,function()
		Elisium:_Animation(Icon,TweenInfo.new(0.75,Enum.EasingStyle.Quint),{
			ImageTransparency = 0.01,
			Size = UDim2.new(0, 200, 0, 200)
		});

		task.delay(0.25,function()
			Elisium:_Animation(Vignette,TweenInfo.new(5),{
				ImageTransparency = 0.2
			});

			task.wait(Duration or 4.5)

			Elisium:_Animation(Vignette,TweenInfo.new(3,Enum.EasingStyle.Quint,Enum.EasingDirection.InOut),{
				Size = UDim2.new(2, 0, 2, 0)
			});

			Elisium:_Animation(Icon,TweenInfo.new(0.75,Enum.EasingStyle.Quint,Enum.EasingDirection.InOut),{
				ImageTransparency = 1,
			});

			Elisium:_Animation(Loader,TweenInfo.new(1.5,Enum.EasingStyle.Quint,Enum.EasingDirection.InOut),{
				BackgroundTransparency = 1
			});

			task.delay(0.1,function()
				Elisium:_Animation(Vignette,TweenInfo.new(1,Enum.EasingStyle.Quint,Enum.EasingDirection.InOut),{
					ImageTransparency = 1
				});
...
function Elisium.newNotify()
	if Elisium.NOTIFY_CACHE then
	...
	Notification.Name = Elisium:_RandomString()
	Notification.Parent = CoreGui
	Notification.ResetOnSpawn = false
	Notification.ZIndexBehavior = Enum.ZIndexBehavior.Global

	NotifyContainer.Name = Elisium:_RandomString()
	...
	Elisium.NOTIFY_CACHE = {
		new = function(Notify: Notify) : NotifyPayback
			Notify = Elisium.__CONFIG(Notify, {
				Icon = Elisium.Logo,
				Title = "Notification",
				Content = "Content",
				Duration = 3,
			...
			BlockFrame.Name = Elisium:_RandomString()
			...
			NotifyFrame.Name = Elisium:_RandomString()
			...
			CompLogo.ZIndex = 4
			CompLogo.Image = Elisium:_GetIcon(Notify.Icon);
			...
			Header.TextColor3 = Elisium.Colors.SwitchColor
			Header.TextSize = 14.000
			Header.TextXAlignment = Enum.TextXAlignment.Left

			Body.Name = Elisium:_RandomString()
			...
			Body.TextColor3 = Elisium.Colors.SwitchColor
			Body.TextSize = 12.000
			Body.TextTransparency = 0.500
			Body.TextXAlignment = Enum.TextXAlignment.Left
			Body.TextYAlignment = Enum.TextYAlignment.Top

			TimeLeftFrame.Name = Elisium:_RandomString()
			...
			TimeLeft.Name = Elisium:_RandomString()
			TimeLeft.Parent = TimeLeftFrame
			TimeLeft.BackgroundColor3 = Elisium.Colors.Highlight
			...
			local UpdateText = function()
				...
				if BlockFrame:GetAttribute('Already') then
					Elisium:_Animation(BlockFrame,TweenInfo.new(0.3),{
			...
			local Close = function()
				Elisium:_Animation(NotifyFrame,TweenInfo.new(0.65,Enum.EasingStyle.Quint),{
			...
			local Show = function()
				Elisium:_Animation(NotifyFrame,TweenInfo.new(0.5,Enum.EasingStyle.Quint),{
					Position = UDim2.new(0,0,0,0)
				});
			end;

			if typeof(Notify.Duration) == 'number' and Notify.Duration ~= math.huge then
				Elisium:_Animation(TimeLeft,TweenInfo.new(Notify.Duration + 0.2,Enum.EasingStyle.Linear),{
			...
			return {
				...
				SetProgress = function(self , new , Time)
					...
					Elisium:_Animation(TimeLeft,TweenInfo.new(Time or 0.85,(Time and Enum.EasingStyle.Linear) or Enum.EasingStyle.Quint),{
