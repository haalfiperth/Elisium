...
local Library do
    ...
    Library.Sections.DependencyBox = function(self, Dependencies)
        ...
        -- Function to get current value from any element type
        local function GetElementValue(Element)
            if not Element then return nil end
            
            -- Try Get() method first
            if Element.Get and type(Element.Get) == "function" then
                local success, value = pcall(function() return Element:Get() end)
            ...
            elseif Element.Toggled ~= nil then
        ...
        -- Function to check if values match based on type
        local function ValuesMatch(CurrentValue, RequiredValue)
            -- Handle nil cases
            ...
            -- Handle table comparisons (for multi-select dropdowns)
            if type(CurrentValue) == "table" then
                -- Check if all required values are in current value
                for _, ReqVal in ipairs(RequiredValue) do
                    local found = false
            ...
            -- Handle range checks for sliders [min, max]
            if type(CurrentValue) == "number" and #RequiredValue == 2 then
                return CurrentValue >= RequiredValue[1] and CurrentValue <= RequiredValue[2]
            end
        ...
        function DependencyBox:Update()
            ...
            for i = 1, #DependencyBox.Dependencies do
                local Dependency = DependencyBox.Dependencies[i]
                ...
                local CurrentValue = GetElementValue(Element)
                
                if not ValuesMatch(CurrentValue, RequiredValue) then
                    ShouldShow = false
                    break
                end
            end
            ...
            -- Also hide/show all children properly
            for _, child in ipairs(Items["DependencyBox"].Instance:GetChildren()) do
        ...
        function DependencyBox:Destroy()
            for _, connection in ipairs(DependencyBox.Connections) do
        ...
        -- Hook into dependency elements to trigger updates
        for i = 1, #DependencyBox.Dependencies do
            local Dependency = DependencyBox.Dependencies[i]
            local Element = Dependency[1]
            ...
            elseif ElementClass == "Keybind" then
                -- Hook Keybind's Press method
                if Element.Press then
            ...
            elseif ElementClass == "Colorpicker" then
                -- Hook Colorpicker's Set method
                if Element.Set then
                    local OriginalSet = Element.Set
                    Element.Set = function(self, Color, Alpha)
                        OriginalSet(self, Color, Alpha)
                        UpdateCallback()
                    end
                end
            end
        ...
        -- Force initial update after a delay to ensure all elements are initialized
        spawn(function()
            wait(0.1)
            DependencyBox:Update()
        end)
