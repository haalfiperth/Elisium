...
local Library do
	...
	Library.Sections.DependencyBox = function(self, Dependencies)
                    ...
                    if Element.Multi then
                        if type(RequiredValue) == "table" then
                            for _, Val in RequiredValue do
                                if not Element.Value[Val] then
                                    ShouldShow = false
                                    break
                                end
                            end
                        elseif not Element.Value[RequiredValue] then
                            ShouldShow = false
                            break
                        end
                    else
        ...
        for _, Dependency in DependencyBox.Dependencies do
            ...
            local ElementClass = Element.Class or ""
            local OriginalFunc = Element.Set or Element.Press
            if OriginalFunc then
                if ElementClass == "Toggle" or ElementClass == "Dropdown" or ElementClass == "Slider" or ElementClass == "Input" or ElementClass == "Textbox" then
                    Element.Set = function(self, ...)
                        OriginalFunc(self, ...)
                        DependencyBox:Update()
                    end
                elseif ElementClass == "Keybind" then
                    Element.Press = function(self, ...)
                        OriginalFunc(self, ...)
                        DependencyBox:Update()
                    end
                end
        ...
        DependencyBox.Elements = {Content = Items["DependencyBox"]}
        DependencyBox:Update()
