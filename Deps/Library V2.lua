...
local Library do
	...
	Library = {
		...
		Folders = {
			Directory = "Elisium",
			Configs = "Elisium/Configs",
			Assets = "Elisium/Assets"
		},
		Images = {
	...
	Library.Sections.DependencyBox = function(self, Dependencies)
        ...
        local CheckFunctions = {
            Toggle = function(Element, Required)
                return Element.Value == Required
            end,
            Keybind = function(Element, Required)
                return Element.Toggled == Required
            end,
            Input = function(Element, Required)
                return Element.Value == Required
            end,
            Textbox = function(Element, Required)
                return Element.Value == Required
            end,
            Dropdown = function(Element, Required)
                if Element.Multi then
                    if type(Required) == "table" then
                        local ElementValue = Element.Value
                        for i = 1, #Required do
                            if not ElementValue[Required[i]] then
                                return false
                            end
                        end
                        return true
                    else
                        return Element.Value[Required] ~= nil
                    end
                else
                    return Element.Value == Required
                end
            end,
            Slider = function(Element, Required)
                if type(Required) == "table" then
                    local Val = Element.Value
                    return Val >= Required[1] and Val <= Required[2]
                else
                    return Element.Value == Required
                end
            end
        }
        local OptimizedChecks = {}
        local DependencyCount = #Dependencies
        for i = 1, DependencyCount do
            local Dependency = Dependencies[i]
            local Element = Dependency[1]
            local Required = Dependency[2]
            if Element then
                local CheckFunc = CheckFunctions[Element.Class]
                if CheckFunc then
                    OptimizedChecks[i] = {Element, Required, CheckFunc}
                end
            end
        end
        local OptimizedCheckCount = #OptimizedChecks
        local FrameInstance = Items["DependencyBox"].Instance
        local SectionResize = DependencyBox.Section.Resize
        function DependencyBox:Update()
            if OptimizedCheckCount == 0 then
                if not DependencyBox.Visible then
                    DependencyBox.Visible = true
                    FrameInstance.Visible = true
                    if SectionResize then
                        SectionResize(DependencyBox.Section)
                    end
                end
                return
            end
            for i = 1, OptimizedCheckCount do
                local Check = OptimizedChecks[i]
                if not Check[3](Check[1], Check[2]) then
                    if DependencyBox.Visible then
        ...
        function DependencyBox:QueueUpdate()
            if not DependencyBox.UpdateQueued then
                DependencyBox.UpdateQueued = true
                task.defer(function()
                    DependencyBox.UpdateQueued = false
                    DependencyBox:Update()
                end)
            end
        end
        function DependencyBox:SetupDependencies(NewDependencies)
            DependencyBox.Dependencies = NewDependencies or {}
            OptimizedChecks = {}
            local Count = #NewDependencies
            for i = 1, Count do
                local Dependency = NewDependencies[i]
                local Element = Dependency[1]
                local Required = Dependency[2]
                if Element then
                    local CheckFunc = CheckFunctions[Element.Class]
                    if CheckFunc then
                        OptimizedChecks[#OptimizedChecks + 1] = {Element, Required, CheckFunc}
                    end
                end
            end
            OptimizedCheckCount = #OptimizedChecks
            DependencyBox:Update()
        end
        function DependencyBox:AddDependency(Element, Value)
            if Element then
                ...
                if CheckFunc then
                    ...
                    OptimizedChecks[#OptimizedChecks + 1] = {Element, Value, CheckFunc}
                    OptimizedCheckCount = OptimizedCheckCount + 1
                    DependencyBox:Update()
                end
            end
        end
        ...
        for i = 1, DependencyCount do
            local Element = Dependencies[i][1]
            if Element and not HookedElements[Element] then
                HookedElements[Element] = true
                local FunctionName = HookConfig[Element.Class]
                if FunctionName then
                    local OriginalFunction = Element[FunctionName]
                    if OriginalFunction then
                        Element[FunctionName] = function(...)
                            OriginalFunction(...)
                            DependencyBox:QueueUpdate()
                        end
                    end
                end
            end
