...
local Library do
    ...
    function Library:CreateDependency(Element, Dependencies)
        if not Element then return end
        
        Library.Dependencies[Element] = Dependencies
        Library:UpdateDependency(Element)
        
        for _, Dependency in Dependencies do
            local DepElement = Dependency[1]
            
            if not Library.DependencyCallbacks[DepElement] then
                Library.DependencyCallbacks[DepElement] = {}
            end
            
            table.insert(Library.DependencyCallbacks[DepElement], Element)
        end
    end
    
    function Library:UpdateDependency(Element)
        local Dependencies = Library.Dependencies[Element]
        ...
        for _, Dependency in Dependencies do
            ...
            if DepElement.Type == "Toggle" then
                if DepElement.Value ~= RequiredValue then
                    ShouldShow = false
                    break
                end
            elseif DepElement.Type == "Dropdown" then
                if type(RequiredValue) == "table" then
                    local HasMatch = false
                    for _, Val in RequiredValue do
                        if type(DepElement.Value) == "table" then
                            if TableFind(DepElement.Value, Val) then
                                HasMatch = true
                                break
                            end
                        elseif DepElement.Value == Val then
                            HasMatch = true
                            break
                        end
                    end
                    ...
                    if type(DepElement.Value) == "table" then
                        if not TableFind(DepElement.Value, RequiredValue) then
                            ShouldShow = false
                            break
                        end
                    elseif DepElement.Value ~= RequiredValue then
                        ShouldShow = false
                        break
                    end
                end
        ...
        if Element.SetVisibility then
            Element:SetVisibility(ShouldShow)
        end
    end
    
    function Library:UpdateDependents(Element)
        local Dependents = Library.DependencyCallbacks[Element]
        if not Dependents then return end
        
        for _, Dependent in Dependents do
            Library:UpdateDependency(Dependent)
        end
    end
    ...
    local Components = { } do
        ...
        Components.Toggle = function(self, Data)
            ...
            function Toggle:Set(Value)
                ...
                Library:UpdateDependents(Toggle)
            end
